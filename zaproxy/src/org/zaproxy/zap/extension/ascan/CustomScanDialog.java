...
public class CustomScanDialog extends StandardFieldsDialog {
	...
	public void init(SiteNode node) {
		...
		//Sequence panel
		this.setCustomTabPanel(5, this.getSequencePanel(true));
	...
	private SequencePanel getSequencePanel(boolean reset) {
		if(this.sequencePanel == null || reset) {
			this.sequencePanel = new SequencePanel(this);
		}
		return this.sequencePanel;
	}
	
	/**
	 * Updates the Recurse checkbox on the main tab of the Custom Scan Dialog, based on selections on the Sequence Panel.
	 */
	...
	public void save() {
		...
		HashMap<Integer, ScriptWrapper> directScanSequenceScripts = getDirectScriptsMapping(getSequencePanel().getSelectedDirectScripts());
		...
		this.extension.startScanCustom(
				...
				selectedIncludeScripts,
				directScanSequenceScripts);
	}
	
	private HashMap<Integer, ScriptWrapper> getDirectScriptsMapping(List<ScriptWrapper> directScripts) 
	{
		HashMap<Integer, ScriptWrapper> mapping = new HashMap<Integer, ScriptWrapper>();
		if(directScripts.size() > 0) {
			ExtensionHistory extHistory = (ExtensionHistory) Control.getSingleton().getExtensionLoader().getExtension(ExtensionHistory.class);
			ExtensionScript extScript = (ExtensionScript) Control.getSingleton().getExtensionLoader().getExtension(ExtensionScript.class);
			SiteNode sequenceNode = new SiteNode(null, 11, "[[Sequences]]");
			for(ScriptWrapper seqScr : directScripts) {
				try {
				SiteNode seqDirectory = new SiteNode(null, HistoryReference.TYPE_HIDDEN, seqScr.getName());
				SequenceScript iSeq = extScript.getInterface(seqScr, SequenceScript.class);
				for(HttpMessage msg : iSeq.getAllRequestsInScript())
				{
					SiteNode temp = new SiteNode(null, HistoryReference.TYPE_HIDDEN, "");
					HistoryReference ref = new HistoryReference(Model.getSingleton().getSession(), HistoryReference.TYPE_HIDDEN, msg);
					msg.setHistoryRef(ref);
					temp.setHistoryReference(ref);
					
					mapping.put(ref.getHistoryId(), seqScr);
					extHistory.addHistory(ref);
					seqDirectory.add(temp);				
				}
				sequenceNode.add(seqDirectory);
				}catch(Exception e) {
					logger.error("Exception occurred when converting ScriptWrappers to SequenceScripts: " + e.getMessage());
				}
			}
