...
public class SequencePanel extends AbstractParamPanel {
	...
	private static final String BTNINCLUDESELECT = "Select all \"Include in scan\"";
	private static final String BTNINCLUDEDESELECT = "Deselect all \"Include in scan\"";
	private static final String BTNDIRECTSELECT = "Select all \"Direct scan\"";
	private static final String BTNDIRECTDESELECT = "Deselect all \"Direct scan\"";
	
	private static final String TBLSEQHEADER0 = "Script name";
	private static final String TBLSEQHEADER1 = "Include in scan";
	private static final String TBLSEQHEADER2 = "Direct scan";
	private static final String TBLSEQHEADER3 = "Script";
	
	private static final String HELPSTRING = "ui.dialogs.sequence";

	private JButton btnInclude = null; 
	private JButton btnDirect = null;
	private JButton btnHelp = null;

	private CustomScanDialog parent = null;
	 ...
	 */
	public SequencePanel(CustomScanDialog parent) {
		creating = true;
		this.parent = parent;
		...
		btnDirect = new JButton(BTNDIRECTSELECT);
		btnDirect.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(btnDirect.getText().equals(BTNDIRECTSELECT))
				{
					setBooleanColumn(2, true);
					btnDirect.setText(BTNDIRECTDESELECT);
				}
				else
				{
					setBooleanColumn(2, false);
					btnDirect.setText(BTNDIRECTSELECT);
				}
			}
		});
		GridBagConstraints gbc_btnDirect = new GridBagConstraints();
		gbc_btnDirect.anchor = GridBagConstraints.NORTHWEST;
		gbc_btnDirect.insets = new Insets(0, 15, 5, 0);
		gbc_btnDirect.gridx = 0;
		gbc_btnDirect.gridy = 2;
		add(btnDirect, gbc_btnDirect);
		...
		tblSequence.addPropertyChangeListener(new PropertyChangeListener() {
		...
		tblSequence.setModel(new DefaultTableModel(
				...
				new String[] {
						TBLSEQHEADER0, TBLSEQHEADER1, TBLSEQHEADER2, TBLSEQHEADER3
				}
				) {
			private static final long serialVersionUID = 1L;
			@SuppressWarnings("rawtypes")
			Class[] columnTypes = new Class[] {
				String.class, Boolean.class, Boolean.class, ScriptWrapper.class
			};
		...
		tblSequence.getColumnModel().getColumn(2).setPreferredWidth(100);
		tblSequence.getColumnModel().getColumn(2).setMinWidth(100);
		tblSequence.getColumnModel().getColumn(3).setPreferredWidth(0);
		tblSequence.getColumnModel().getColumn(3).setMinWidth(0);
		tblSequence.getColumnModel().getColumn(3).setMaxWidth(0);
	 ...
	 * @return A list of the selected Sequence scripts int eh "Include" column.
	 */
	public List<ScriptWrapper> getSelectedIncludeScripts() {
		...
		for(int i = 0; i < tblSequence.getModel().getRowCount(); i++) {
			...
			if(selected) {
				ScriptWrapper script = (ScriptWrapper)tblSequence.getValueAt(i, 3);
				sw.add(script);
			}
		}
		return sw;
	...
	/**
	 * Gets a list of Sequence Scripts, that were selected in the "Direct" column.
	 * @return A list of the selected Sequence scripts int eh "Direct" column.
	 */
	public List<ScriptWrapper> getSelectedDirectScripts() {
		ArrayList<ScriptWrapper> sw = new ArrayList<ScriptWrapper>();
		for(int i = 0; i < tblSequence.getModel().getRowCount(); i++) {
			boolean selected = (boolean)tblSequence.getValueAt(i, 2);
			if(selected) {
				ScriptWrapper script = (ScriptWrapper)tblSequence.getValueAt(i, 3);
				sw.add(script);
			}
